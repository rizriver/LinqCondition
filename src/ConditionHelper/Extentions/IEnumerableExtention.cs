...
namespace RizRiver.ConditionHelper
{
    ...
    public static class IEnumerableExtention
    {
        ...
        /// <returns>条件を満たす、入力シーケンスの要素を含む System.Collections.Generic.IEnumerable&lt;T&gt;。</returns>
        public static IEnumerable<TSource> Where<TSource>(this IEnumerable<TSource> source, ConditionGroup conditionGroup)
        {
            ...
            return source.Where(ExpressionUtil.CreateConditionPredicate<TSource>(conditionGroup).Compile());
        }
        ...
        /// <param name="conditionGroup">条件</param>
        /// <returns>指定された条件でソース シーケンスの要素がテストに合格する場合は true。それ以外の場合は false。</returns>
        public static bool Any<TSource>(this IEnumerable<TSource> source, ConditionGroup conditionGroup)
        {
            if (conditionGroup == null || conditionGroup.GroupList.Count == 0)
            {
                return System.Linq.Enumerable.Any(source);
            ...
            return source.Any(ExpressionUtil.CreateConditionPredicate<TSource>(conditionGroup).Compile());
        }
        ...
        /// <param name="conditionGroup">条件</param>
        /// <returns>条件を満たす、シーケンス内の要素数を表す数値。</returns>
        public static int Count<TSource>(this IEnumerable<TSource> source, ConditionGroup conditionGroup)
        {
            if (conditionGroup == null || conditionGroup.GroupList.Count == 0)
            {
                return System.Linq.Enumerable.Count(source);
            ...
            return source.Count(ExpressionUtil.CreateConditionPredicate<TSource>(conditionGroup).Compile());
        }
    }
}
