...
namespace RizRiver.Linq.LinqCondition.Utils
{
    /// <summary>
    /// 式ツリーを操作するユーティリティクラス
    /// </summary>
    internal static class ExpressionUtil
    {
        ...
        /// <returns>構築された式ツリー</returns>
        public static Expression<Func<TSource, bool>> CreateConditionPredicate<TSource>(ConditionChain conditionChain)
        {
            ParameterExpression param = Expression.Parameter(typeof(TSource), "e");
        ...
        private static Expression CreateBodyExpression(ParameterExpression param, Condition condition)
        {
            ...
            foreach (object conditionValue in condition.Values)
            {
                ...
                if (value.GetType() != convertType)
                {
                    ...
                    {
                        ArgumentException newException = new ArgumentException("Condition value type is invalid.", "condition", ex);
                        newException.Data.Add("InvalidCondition", condition);
                        throw newException;
                    }
                }
